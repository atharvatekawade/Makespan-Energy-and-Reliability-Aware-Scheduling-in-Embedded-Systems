import numpy as np
import config, utils
import math
from copy import deepcopy

def my(sp, fp, step, l = []):
    th_list = np.arange(sp, fp + step, step).tolist()
    if(l == []):
        l, _ = utils.urv()

    od = []
    idx = []

    for i in range(len(config.graph)):
        idx.append(0)
        t = 0
        for j in range(len(config.times)):
            t += config.times[j][l[i]]
        
        m = 0
        for j in range(len(config.graph)):
            if(config.graph[j][i] > 0):
                m = max(m, config.graph[j][i])

        od.append(t/len(config.times)+m)     
    
    u = [[i, od[i]] for i in range(len(config.graph))]
    
    for i in range(len(config.graph)):
        for j in range(len(config.graph)-i-1):
            if(u[j][1] < u[j + 1][1]):
                tempo = u[j]
                u[j]= u[j + 1]
                u[j + 1]= tempo
            
            elif(u[j][1] == u[j + 1][1] and l[j] < l[j+1]):
                tempo = u[j]
                u[j]= u[j + 1]
                u[j + 1]= tempo
  
    v = [u[i][0] for i in range(len(config.graph))]

    fin = float('inf')
    rel = 0
    ene = float('inf')
    final = [0] * len(config.graph)

    assert(len(v) == len(config.graph))

    for i in range(len(v)):
        if(i not in v):
            print("Vertex missing")
            quit()

        idx[v[i]] = i

    for a in range(-1, len(config.graph)):
        for th in th_list:
            fins = []
            avail = []
            alloc =[]

            e = 0
            r = 1

            for i in range(len(config.times)):
                avail.append(0)
            for i in range(len(l)):
                fins.append(0)
                alloc.append(-1)

            for i in range(len(l)):
                metrics = []
                for j in range(len(config.times)):
                    s1 = avail[j]
                    for k in range(len(config.graph)):
                        if(config.graph[k][l[i]] > 0):
                            comm = config.graph[k][l[i]]
                            if(j == alloc[k]):
                                comm = 0
                            if(fins[k] == 0):
                                print("Did not allocate predecessor:",l[i],k)
                                quit()
                            s1 = max(s1, fins[k]+comm)

                    f1 = s1 + config.times[j][l[i]]
                    e1 = (config.PS[j] + config.C[j]*(config.F[j]**config.A[j]))*config.times[j][l[i]]/config.F[j]
                    lf = config.L[j]*10**(config.D[j]*(1-config.F[j])/(1-config.F_MIN[j]))
                    r1 = math.exp(-lf*config.times[j][l[i]]/config.F[j])
                    metrics.append([j, f1, e1, r1])

                fmax = 0
                tmax = 0
                rmax = 0
                emax = 0
                fmin = float('inf')
                rmin = float('inf')
                tmin = float('inf')
                emin = float('inf')

                for j in range(len(metrics)):
                    fmax = max(fmax, metrics[j][1])
                    tmax = max(tmax, config.times[j][l[i]])
                    fmin = min(fmin, metrics[j][1])
                    emax = max(emax, metrics[j][2])
                    rmax = max(rmax, metrics[j][3])
                    rmin = min(rmin, metrics[j][3])
                    tmin = min(tmin, config.times[j][l[i]])
                    emin = min(emin, metrics[j][2])  

                min_metric = float('inf')
                j1 = -1
                for j in range(len(metrics)):
                    if(idx[l[i]] <= a):
                        metric = th*(metrics[j][1]-fmin)/(fmax-fmin) + (1 - th)*((rmax-metrics[j][3])/(rmax-rmin))
                    else:
                        metric = metrics[j][2]

                    if(metric < min_metric):
                        min_metric = metric
                        j1 = j
                                
                fins[l[i]] = metrics[j1][1]
                alloc[l[i]] = metrics[j1][0]
                avail[alloc[l[i]]] = fins[l[i]]

                e = e + metrics[j1][2]
                r = r * metrics[j1][3]

            config.my_alloc.append(alloc)
            config.my_results.append([max(fins), r, e])
    
    return final, fin, rel, ene

def my_output(R, E):
    f1 = float('inf')
    r1 = 0
    e1 = float('inf')
    final_alloc1 = None

    f2 = float('inf')
    r2 = 0
    e2 = float('inf')
    final_alloc2 = None


    for i in range(len(config.my_results)):
        alloc = config.my_alloc[i]
        f, r, e = config.my_results[i]
        if(R-r < config.epsilon):
            freq = utils.soea1(alloc, R)
            f, r, e = utils.simulate(alloc, freq = freq)
            if(e <= E):
                if(f < f1):
                    f1 = f
                    r1 = r
                    e1 = e
                    final_alloc1 = deepcopy(alloc)
                    
                elif(f == f1 and e < e1):
                    f1 = f
                    r1 = r
                    e1 = e
                    final_alloc1 = deepcopy(alloc)
            
            if(e < e2):
                f2 = f
                r2 = r
                e2 = e
                final_alloc2 = deepcopy(alloc)

            elif(e == e2 and f < f2):
                f2 = f
                r2 = r
                e2 = e
                final_alloc2 = deepcopy(alloc)

    if(f1 == float('inf')):
        f1 = f2
        r1 = r2
        e1 = e2
        final_alloc1 = final_alloc2   

    return final_alloc1, f1, r1, e1